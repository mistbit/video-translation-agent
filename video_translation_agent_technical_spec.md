...
1. 接入层：CLI、Web UI、内部工具调用入口
2. 控制层：API、编排器、任务状态管理
3. 计算层：字幕提取、翻译、TTS、渲染等 Worker
...
- `local`：直接调用本地编排器和 pipeline
- `remote`：通过 HTTP 调用服务端 API
...
| `POST` | `/api/v1/jobs/{job_id}/segments/{segment_id}/rerun` | 重跑指定片段 |
...
建议采用单机多 Worker 架构，适用于内部验证与低并发处理场景。
...
| 状态依赖 | PostgreSQL、Redis | 优先容器化，或接入已有内部实例 |
| 模型服务依赖 | vLLM、ASR/TTS/OCR 推理服务 | 独立服务化，优先容器化 |
| 文件存储依赖 | 本地磁盘、NAS、MinIO | 使用宿主机挂载卷或独立存储服务 |
| 监控依赖 | Prometheus、Grafana、日志采集 | 第二阶段按需接入 |

推荐原则如下：

- 应容器化的内容：应用运行时、API、Worker、数据库、缓存、中间服务
...
第一阶段建议直接提供一份 `docker-compose.yml` 作为标准启动入口。  
推荐通过 Compose 管理以下内容：
...
1. 先在单机 Linux GPU 环境验证裸机链路
2. 将 API、Worker、PostgreSQL、Redis 迁移到 `docker compose`
3. 将模型目录、工作目录、缓存目录改造为宿主机挂载卷
4. 稳定后再拆分 `worker-cpu` 与 `worker-gpu`
5. 当并发、稳定性、资源隔离需求上升时，再升级到 Kubernetes 或内部调度平台

结论上：

- Docker 非常适合作为第一阶段与第二阶段的标准部署方式
- 对该项目而言，推荐“容器化应用 + 宿主机挂载模型与产物 + GPU Worker 独立运行”的组合
- 如果目标是快速建立可复制、可维护的内部环境，`docker compose` 是当前最合适的起步方案

### 13.11 面向 MacBook M4 宿主机的优化建议
